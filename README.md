...
GitHub 仓库：[dingzhenxiaoama-cpu/GaN-HEMT-Agent-](https://github.com/dingzhenxiaoama-cpu/GaN-HEMT-Agent-)
...
> 当前版本已完成 LangGraph 主流程、四类测量数据 CSV 分析、国产大模型 Mock/Live 调用、ASM-HEMT 模型选择和参数组规划。MeQLab/MCP、真实参数拟合、ASM-HEMT 模型卡和物理 QA 尚未接入。
...
- `select_model` ASM-HEMT 模型选择和参数组规划节点；
- CSV 数据读取；
...
- API 密钥缺失时的明确错误提示；
- 根据已有测量类型自动选择 DC、CV、热、陷阱和高频参数组；
- 记录模型选择理由和大模型建议是否参与。

当前参数组中的名称（如 `vth0`、`rth`、`trap_strength`）是项目内部规划名称，仅用于工作流演示和参数分组，尚未映射到 MeQLab 提供的官方 ASM-HEMT 参数名。

## 当前验证状态

项目已经在本地虚拟环境中完成端到端原型运行，验证结果包括：

- 四类演示 CSV 均可正确加载和统计；
- 大模型状态成功显示为 `live`，证明 `.env`、模型 API 地址和密钥配置已生效；
- 大模型能够根据数据摘要提出 DC、C-V、Pulse I-V 和 S 参数的分阶段建模建议；
- 规则节点固定选择 `ASM-HEMT`，并生成 `dc`、`cv`、`thermal`、`trap`、`high_frequency` 参数组；
- 拟合顺序被规划为 `DC → CV → Thermal → Trap → High Frequency`。

这里的输出仍然是“数据摘要、决策建议和拟合计划”，并不代表已经提取出真实 ASM-HEMT 参数，也不包含 MeQLab 仿真结果或有效 RMSE。
...
真实模型运行还会输出 `模型选择` 字段，其中包含目标模型、可用数据类型、参数组、内部候选参数、拟合顺序和选择理由。
...
```text
select_model → MCP 会话 → MeQLab 创建 ASM-HEMT 模型 → DC I-V 拟合 → 返回参数、收敛状态和 RMSE
```

开始真实接入前，需要从组委会或概伦获得：

- MCP 传输方式（Streamable HTTP 或 stdio）；
- 服务地址或本地启动命令；
- 鉴权方式；
- 实际工具名称和输入输出 Schema；
- 测试数据集的服务端访问方式；
- MeQLab 会话生命周期和 ASM-HEMT 示例调用。

在真实服务信息到位前，先实现明确标记 `source=mock` 的 MCP 模拟层、工具探测程序和 `fit_dc` 节点。拿到服务后再按实际工具 Schema 替换模拟调用，不能假设服务器一定提供某个自定义工具名。
...
- [x] 配置真实模型服务并完成 API 联调。
- [ ] 增加 MCP 客户端和 MeQLab 工具封装。
- [x] 增加 ASM-HEMT 模型选择节点和内部参数组元数据。
- [ ] 将内部参数组映射到 MeQLab 官方 ASM-HEMT 参数名。
- [ ] 增加分阶段拟合与自动重试。
- [ ] 增加物理 QA 规则。
- [ ] 生成模型卡、运行日志和运行总结。
- [ ] 使用隐藏测试流程进行端到端验证。
